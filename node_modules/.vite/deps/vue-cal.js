...
var O = class {
  constructor(e2) {
    ...
    __publicField(this, "selectCell", (e22 = false, t2, s2) => {
      this._vuecal.$emit("cell-click", s2 ? { date: t2, split: s2 } : t2), this._vuecal.clickToNavigate || e22 ? this._vuecal.switchToNarrowerView() : this._vuecal.dblclickToNavigate && "ontouchstart" in window && (this._vuecal.domEvents.dblTapACell.taps++, setTimeout(() => this._vuecal.domEvents.dblTapACell.taps = 0, this._vuecal.domEvents.dblTapACell.timeout), this._vuecal.domEvents.dblTapACell.taps >= 2 && (this._vuecal.domEvents.dblTapACell.taps = 0, this._vuecal.switchToNarrowerView(), this._vuecal.$emit("cell-dblclick", s2 ? { date: t2, split: s2 } : t2)));
    });
    __publicField(this, "keyPressEnterCell", (e22, t2) => {
      this._vuecal.$emit("cell-keypress-enter", t2 ? { date: e22, split: t2 } : e22), this._vuecal.switchToNarrowerView();
    });
    __publicField(this, "getPosition", (e22) => {
      const { left: t2, top: s2 } = this._vuecal.cellsEl.getBoundingClientRect(), { clientX: i2, clientY: n2 } = "ontouchstart" in window && e22.touches ? e22.touches[0] : e22;
      return { x: i2 - t2, y: n2 - s2 };
    });
    __publicField(this, "minutesAtCursor", (e22) => {
      let t2 = 0, s2 = { x: 0, y: 0 };
      const { timeStep: i2, timeCellHeight: n2, timeFrom: a2 } = this._vuecal.$props;
      return "number" == typeof e22 ? t2 = e22 : "object" == typeof e22 && (s2 = this.getPosition(e22), t2 = Math.round(s2.y * i2 / parseInt(n2) + a2)), { minutes: Math.max(Math.min(t2, 1440), 0), cursorCoords: s2 };
    });
    this._vuecal = e2;
  }
};
...
var j = { class: "vuecal__flex vuecal__weekdays-headings" };
var A = ["onClick"];
var V = { class: "vuecal__flex weekday-label", grow: "" };
var Y = { class: "full" };
var L = { class: "small" };
var F = { class: "xsmall" };
var B = { key: 0 };
var N = { key: 0, class: "vuecal__flex vuecal__split-days-headers", grow: "" };
var z = (e2, t2) => {
...
var I = z({ inject: ["vuecal", "utils", "view"], props: { transitionDirection: { type: String, default: "right" }, weekDays: { type: Array, default: () => [] }, switchToNarrowerView: { type: Function, default: () => {
  ...
  return openBlock(), createElementBlock("div", j, [(openBlock(true), createElementBlock(Fragment, null, renderList(g2.headings, (y3, D3) => (openBlock(), createElementBlock(Fragment, { key: D3 }, [y3.hide ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", { key: 0, class: normalizeClass(["vuecal__flex vuecal__heading", { today: y3.today, clickable: g2.cellHeadingsClickable }]), style: normalizeStyle(g2.weekdayCellStyles), onClick: (e2) => "week" === g2.view.id && g2.selectCell(y3.date, e2), onDblclick: m2[0] || (m2[0] = (e2) => "week" === g2.view.id && g2.vuecal.dblclickToNavigate && w2.switchToNarrowerView()) }, [createVNode(Transition, { name: `slide-fade--${w2.transitionDirection}`, appear: g2.vuecal.transitions }, { default: withCtx(() => [(openBlock(), createElementBlock("div", { class: "vuecal__flex", column: "", key: !!g2.vuecal.transitions && `${D3}-${y3.dayOfMonth}` }, [createBaseVNode("div", V, [renderSlot(p2.$slots, "weekday-heading", { heading: g2.cleanupHeading(y3), view: g2.view }, () => [createBaseVNode("span", Y, toDisplayString(y3.full), 1), createBaseVNode("span", L, toDisplayString(y3.small), 1), createBaseVNode("span", F, toDisplayString(y3.xsmall), 1), y3.dayOfMonth ? (openBlock(), createElementBlock("span", B, "\xA0" + toDisplayString(y3.dayOfMonth), 1)) : createCommentVNode("", true)])]), g2.vuecal.hasSplits && g2.vuecal.stickySplitLabels ? (openBlock(), createElementBlock("div", N, [(openBlock(true), createElementBlock(Fragment, null, renderList(g2.vuecal.daySplits, (s2, i2) => (openBlock(), createElementBlock("div", { class: normalizeClass(["day-split-header", s2.class || false]), key: i2 }, [renderSlot(p2.$slots, "split-label", { split: s2, view: g2.view }, () => [createTextVNode(toDisplayString(s2.label), 1)])], 2))), 128))])) : createCommentVNode("", true)]))]), _: 2 }, 1032, ["name", "appear"])], 46, A))], 64))), 128))]);
}]]);
var P = { class: "vuecal__header" };
var U = { key: 0, class: "vuecal__flex vuecal__menu", role: "tablist", "aria-label": "Calendar views navigation" };
var R = ["onDragenter", "onDragleave", "onClick", "aria-label"];
var q = { key: 1, class: "vuecal__title-bar" };
var K = ["aria-label"];
var X = { class: "vuecal__flex vuecal__title", grow: "" };
var G = ["aria-label"];
var Z = { key: 0, class: "vuecal__flex vuecal__split-days-headers" };
var J = z({ inject: ["vuecal", "previous", "next", "switchView", "updateSelectedDate", "modules", "view"], components: { WeekdaysHeadings: I }, props: { options: { type: Object, default: () => ({}) }, editEvents: { type: Object, required: true }, hasSplits: { type: [Boolean, Number], default: false }, daySplits: { type: Array, default: () => [] }, viewProps: { type: Object, default: () => ({}) }, weekDays: { type: Array, default: () => [] }, switchToNarrowerView: { type: Function, default: () => {
  ...
  const k2 = resolveComponent("weekdays-headings");
  return openBlock(), createElementBlock("div", P, [g2.options.hideViewSelector ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", U, [(openBlock(true), createElementBlock(Fragment, null, renderList(g2.viewProps.views, (i2, l2) => (openBlock(), createElementBlock(Fragment, { key: l2 }, [i2.enabled ? (openBlock(), createElementBlock("button", { key: 0, class: normalizeClass(["vuecal__view-btn", { "vuecal__view-btn--active": b2.view.id === l2, "vuecal__view-btn--highlighted": a2.highlightedControl === l2 }]), type: "button", onDragenter: (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragEnter(e2, l2, a2.$data), onDragleave: (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragLeave(e2, l2, a2.$data), onClick: (e2) => b2.switchView(l2, null, true), "aria-label": `${i2.label} view` }, toDisplayString(i2.label), 43, R)) : createCommentVNode("", true)], 64))), 128))])), g2.options.hideTitleBar ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", q, [createBaseVNode("button", { class: normalizeClass(["vuecal__arrow vuecal__arrow--prev", { "vuecal__arrow--highlighted": "previous" === a2.highlightedControl }]), type: "button", onClick: D2[0] || (D2[0] = (...e2) => b2.previous && b2.previous(...e2)), onDragenter: D2[1] || (D2[1] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragEnter(e2, "previous", a2.$data)), onDragleave: D2[2] || (D2[2] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragLeave(e2, "previous", a2.$data)), "aria-label": `Previous ${b2.view.id}` }, [renderSlot(a2.$slots, "arrow-prev")], 42, K), createBaseVNode("div", X, [createVNode(Transition, { name: g2.options.transitions ? `slide-fade--${b2.transitionDirection}` : "" }, { default: withCtx(() => [(openBlock(), createBlock(resolveDynamicComponent(b2.broaderView ? "button" : "span"), { type: !!b2.broaderView && "button", key: `${b2.view.id}${b2.view.startDate.toString()}`, onClick: D2[3] || (D2[3] = (e2) => !!b2.broaderView && b2.switchToBroaderView()), "aria-label": !!b2.broaderView && `Go to ${b2.broaderView} view` }, { default: withCtx(() => [renderSlot(a2.$slots, "title")]), _: 3 }, 8, ["type", "aria-label"]))]), _: 3 }, 8, ["name"])]), g2.options.todayButton ? (openBlock(), createElementBlock("button", { key: 0, class: normalizeClass(["vuecal__today-btn", { "vuecal__today-btn--highlighted": "today" === a2.highlightedControl }]), type: "button", onClick: D2[4] || (D2[4] = (...e2) => b2.goToToday && b2.goToToday(...e2)), onDragenter: D2[5] || (D2[5] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragEnter(e2, "today", a2.$data)), onDragleave: D2[6] || (D2[6] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragLeave(e2, "today", a2.$data)), "aria-label": "Today" }, [renderSlot(a2.$slots, "today-button")], 34)) : createCommentVNode("", true), createBaseVNode("button", { class: normalizeClass(["vuecal__arrow vuecal__arrow--next", { "vuecal__arrow--highlighted": "next" === a2.highlightedControl }]), type: "button", onClick: D2[7] || (D2[7] = (...e2) => b2.next && b2.next(...e2)), onDragenter: D2[8] || (D2[8] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragEnter(e2, "next", a2.$data)), onDragleave: D2[9] || (D2[9] = (e2) => g2.editEvents.drag && b2.dnd && b2.dnd.viewSelectorDragLeave(e2, "next", a2.$data)), "aria-label": `Next ${b2.view.id}` }, [renderSlot(a2.$slots, "arrow-next")], 42, G)])), g2.viewProps.weekDaysInHeader ? (openBlock(), createBlock(k2, { key: 2, "week-days": g2.weekDays, "transition-direction": b2.transitionDirection, "switch-to-narrower-view": g2.switchToNarrowerView }, createSlots({ _: 2 }, [a2.$slots["weekday-heading"] ? { name: "weekday-heading", fn: withCtx(({ heading: e2, view: t2 }) => [renderSlot(a2.$slots, "weekday-heading", { heading: e2, view: t2 })]) } : void 0, a2.$slots["split-label"] ? { name: "split-label", fn: withCtx(({ split: e2 }) => [renderSlot(a2.$slots, "split-label", { split: e2, view: b2.view })]) } : void 0]), 1032, ["week-days", "transition-direction", "switch-to-narrower-view"])) : createCommentVNode("", true), createVNode(Transition, { name: `slide-fade--${b2.transitionDirection}` }, { default: withCtx(() => [b2.showDaySplits ? (openBlock(), createElementBlock("div", Z, [(openBlock(true), createElementBlock(Fragment, null, renderList(g2.daySplits, (s2, i2) => (openBlock(), createElementBlock("div", { class: normalizeClass(["day-split-header", s2.class || false]), key: i2 }, [renderSlot(a2.$slots, "split-label", { split: s2, view: b2.view.id }, () => [createTextVNode(toDisplayString(s2.label), 1)])], 2))), 128))])) : createCommentVNode("", true)]), _: 3 }, 8, ["name"])]);
}]]);
var Q = ["draggable"];
var ee = { inject: ["vuecal", "utils", "modules", "view", "domEvents", "editEvents"], props: { cellFormattedDate: { type: String, default: "" }, event: { type: Object, default: () => ({}) }, cellEvents: { type: Array, default: () => [] }, overlaps: { type: Array, default: () => [] }, eventPosition: { type: Number, default: 0 }, overlapsStreak: { type: Number, default: 0 }, allDay: { type: Boolean, default: false } }, data: () => ({ touch: { dragThreshold: 30, startX: 0, startY: 0, dragged: false } }), methods: { onMouseDown(e2, t2 = false) {
...
var te = ["data-split", "aria-label", "onTouchstart", "onMousedown", "onDragover", "onDrop"];
var se = { key: 1, class: "vuecal__cell-events" };
var ie = ["title"];
var ne = z({ inject: ["vuecal", "utils", "modules", "view", "domEvents"], components: { Event: z(ee, [["render", function(s2, i2, l2, r2, o2, d2) {
  return openBlock(), createElementBlock("div", { class: normalizeClass(["vuecal__event", d2.eventClasses]), style: normalizeStyle(d2.eventStyles), tabindex: "0", onFocus: i2[4] || (i2[4] = (...e2) => d2.focusEvent && d2.focusEvent(...e2)), onKeypress: i2[5] || (i2[5] = withKeys(withModifiers((...e2) => d2.onEnterKeypress && d2.onEnterKeypress(...e2), ["stop"]), ["enter"])), onMouseenter: i2[6] || (i2[6] = (...e2) => d2.onMouseEnter && d2.onMouseEnter(...e2)), onMouseleave: i2[7] || (i2[7] = (...e2) => d2.onMouseLeave && d2.onMouseLeave(...e2)), onTouchstart: i2[8] || (i2[8] = withModifiers((...e2) => d2.onTouchStart && d2.onTouchStart(...e2), ["stop"])), onMousedown: i2[9] || (i2[9] = (e2) => d2.onMouseDown(e2)), onMouseup: i2[10] || (i2[10] = (...e2) => d2.onMouseUp && d2.onMouseUp(...e2)), onTouchend: i2[11] || (i2[11] = (...e2) => d2.onMouseUp && d2.onMouseUp(...e2)), onTouchmove: i2[12] || (i2[12] = (...e2) => d2.onTouchMove && d2.onTouchMove(...e2)), onDblclick: i2[13] || (i2[13] = (...e2) => d2.onDblClick && d2.onDblClick(...e2)), draggable: d2.draggable, onDragstart: i2[14] || (i2[14] = (e2) => d2.draggable && d2.onDragStart(e2)), onDragend: i2[15] || (i2[15] = (e2) => d2.draggable && d2.onDragEnd()) }, [d2.vuecal.editEvents.delete && l2.event.deletable ? (openBlock(), createElementBlock("div", { key: 0, class: "vuecal__event-delete", onClick: i2[0] || (i2[0] = withModifiers((...e2) => d2.deleteEvent && d2.deleteEvent(...e2), ["stop"])), onTouchstart: i2[1] || (i2[1] = withModifiers((...e2) => d2.touchDeleteEvent && d2.touchDeleteEvent(...e2), ["stop"])) }, toDisplayString(d2.vuecal.texts.deleteEvent), 33)) : createCommentVNode("", true), renderSlot(s2.$slots, "event", { event: l2.event, view: d2.view.id }), d2.resizable ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__event-resize-handle", contenteditable: "false", onMousedown: i2[2] || (i2[2] = withModifiers((...e2) => d2.onResizeHandleMouseDown && d2.onResizeHandleMouseDown(...e2), ["stop", "prevent"])), onTouchstart: i2[3] || (i2[3] = withModifiers((...e2) => d2.onResizeHandleMouseDown && d2.onResizeHandleMouseDown(...e2), ["stop", "prevent"])) }, null, 32)) : createCommentVNode("", true)], 46, Q);
  ...
  const y2 = resolveComponent("event");
  return openBlock(), createBlock(TransitionGroup, { class: normalizeClass(["vuecal__cell", w2.cellClasses]), name: `slide-fade--${w2.transitionDirection}`, tag: "div", appear: d2.options.transitions, style: normalizeStyle(w2.cellStyles) }, { default: withCtx(() => [(openBlock(true), createElementBlock(Fragment, null, renderList(w2.splitsCount ? w2.splits : 1, (h3, v3) => (openBlock(), createElementBlock("div", { class: normalizeClass(["vuecal__flex vuecal__cell-content", w2.splitsCount && w2.splitClasses(h3)]), key: d2.options.transitions ? `${w2.view.id}-${d2.data.content}-${v3}` : v3, "data-split": !!w2.splitsCount && h3.id, column: "", tabindex: "0", "aria-label": d2.data.content, onFocus: r2[0] || (r2[0] = (e2) => w2.onCellFocus(e2)), onKeypress: r2[1] || (r2[1] = withKeys((e2) => w2.onCellkeyPressEnter(e2), ["enter"])), onTouchstart: (e2) => !w2.isDisabled && w2.onCellTouchStart(e2, w2.splitsCount ? h3.id : null), onMousedown: (e2) => !w2.isDisabled && w2.onCellMouseDown(e2, w2.splitsCount ? h3.id : null), onClick: r2[2] || (r2[2] = (e2) => !w2.isDisabled && w2.onCellClick(e2)), onDblclick: r2[3] || (r2[3] = (e2) => !w2.isDisabled && w2.onCellDblClick(e2)), onContextmenu: r2[4] || (r2[4] = (e2) => !w2.isDisabled && d2.options.cellContextmenu && w2.onCellContextMenu(e2)), onDragenter: r2[5] || (r2[5] = (e2) => !w2.isDisabled && d2.editEvents.drag && w2.dnd && w2.dnd.cellDragEnter(e2, l2.$data, d2.data.startDate)), onDragover: (e2) => !w2.isDisabled && d2.editEvents.drag && w2.dnd && w2.dnd.cellDragOver(e2, l2.$data, d2.data.startDate, w2.splitsCount ? h3.id : null), onDragleave: r2[6] || (r2[6] = (e2) => !w2.isDisabled && d2.editEvents.drag && w2.dnd && w2.dnd.cellDragLeave(e2, l2.$data, d2.data.startDate)), onDrop: (e2) => !w2.isDisabled && d2.editEvents.drag && w2.dnd && w2.dnd.cellDragDrop(e2, l2.$data, d2.data.startDate, w2.splitsCount ? h3.id : null) }, [w2.isWeekOrDayView && !d2.allDay && w2.specialHours.length ? (openBlock(true), createElementBlock(Fragment, { key: 0 }, renderList(w2.specialHours, (s2, i2) => (openBlock(), createElementBlock("div", { class: normalizeClass(["vuecal__special-hours", `vuecal__special-hours--day${s2.day} ${s2.class}`]), style: normalizeStyle(`height: ${s2.height}px;top: ${s2.top}px`) }, null, 6))), 256)) : createCommentVNode("", true), renderSlot(l2.$slots, "cell-content", { events: w2.events, selectCell: (e2) => w2.selectCell(e2, true), split: !!w2.splitsCount && h3 }), w2.eventsCount && (w2.isWeekOrDayView || "month" === w2.view.id && d2.options.eventsOnMonthView) ? (openBlock(), createElementBlock("div", se, [(openBlock(true), createElementBlock(Fragment, null, renderList(w2.splitsCount ? h3.events : w2.events, (t2, s2) => (openBlock(), createBlock(y2, { key: s2, "cell-formatted-date": d2.data.formattedDate, event: t2, "all-day": d2.allDay, "cell-events": w2.splitsCount ? h3.events : w2.events, overlaps: ((w2.splitsCount ? h3.overlaps[t2._eid] : l2.cellOverlaps[t2._eid]) || []).overlaps, "event-position": ((w2.splitsCount ? h3.overlaps[t2._eid] : l2.cellOverlaps[t2._eid]) || []).position, "overlaps-streak": w2.splitsCount ? h3.overlapsStreak : l2.cellOverlapsStreak }, { event: withCtx(({ event: e2, view: t3 }) => [renderSlot(l2.$slots, "event", { view: t3, event: e2 })]), _: 2 }, 1032, ["cell-formatted-date", "event", "all-day", "cell-events", "overlaps", "event-position", "overlaps-streak"]))), 128))])) : createCommentVNode("", true)], 42, te))), 128)), w2.timelineVisible ? (openBlock(), createElementBlock("div", { class: "vuecal__now-line", style: normalizeStyle(`top: ${w2.todaysTimePosition}px`), key: d2.options.transitions ? `${w2.view.id}-now-line` : "now-line", title: w2.utils.date.formatTime(w2.vuecal.now) }, null, 12, ie)) : createCommentVNode("", true)]), _: 3 }, 8, ["class", "name", "appear", "style"]);
}]]);
...
var le = z({ inject: ["vuecal", "view", "editEvents"], components: { "vuecal-cell": ne }, props: { options: { type: Object, required: true }, cells: { type: Array, required: true }, label: { type: String, required: true }, daySplits: { type: Array, default: () => [] }, shortEvents: { type: Boolean, default: true }, height: { type: String, default: "" }, cellOrSplitMinWidth: { type: Number, default: null } }, computed: { hasCellOrSplitWidth() {
  ...
  const g2 = resolveComponent("vuecal-cell");
  return openBlock(), createElementBlock("div", { class: "vuecal__flex vuecal__all-day", style: normalizeStyle(v2.cellOrSplitMinWidth && { height: v2.height }) }, [v2.cellOrSplitMinWidth ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", ae, [createBaseVNode("span", null, toDisplayString(v2.label), 1)])), createBaseVNode("div", { class: normalizeClass(["vuecal__flex vuecal__cells", `${D2.view.id}-view`]), grow: "", style: normalizeStyle(v2.cellOrSplitMinWidth ? `min-width: ${v2.cellOrSplitMinWidth}px` : "") }, [(openBlock(true), createElementBlock(Fragment, null, renderList(v2.cells, (t2, s2) => (openBlock(), createBlock(g2, { key: s2, options: v2.options, "edit-events": D2.editEvents, data: t2, "all-day": true, "cell-width": v2.options.hideWeekdays.length && (D2.vuecal.isWeekView || D2.vuecal.isMonthView) && D2.vuecal.cellWidth, "min-timestamp": v2.options.minTimestamp, "max-timestamp": v2.options.maxTimestamp, "cell-splits": v2.daySplits }, { event: withCtx(({ event: e2, view: t3 }) => [renderSlot(l2.$slots, "event", { view: t3, event: e2 })]), _: 2 }, 1032, ["options", "edit-events", "data", "cell-width", "min-timestamp", "max-timestamp", "cell-splits"]))), 128))], 6)], 4);
}]]);
var re = ["lang"];
var oe = createTextVNode("\xA0");
var de = createBaseVNode("i", { class: "angle" }, null, -1);
var ue = createTextVNode("\xA0");
var he = createTextVNode("\xA0");
var ce = createBaseVNode("i", { class: "angle" }, null, -1);
var ve = createTextVNode("\xA0");
var pe = { class: "default" };
var me = { key: 0, class: "vuecal__flex vuecal__body", grow: "" };
var we = ["onBlur", "innerHTML"];
var ye = ["innerHTML"];
var De = ["innerHTML"];
var ge = { class: "vuecal__flex", row: "", grow: "" };
var fe = { key: 0, class: "vuecal__time-column" };
var _e = createBaseVNode("span", { class: "vuecal__time-cell-line" }, null, -1);
var be = { class: "vuecal__time-cell-label" };
var ke = { key: 1, class: "vuecal__flex vuecal__week-numbers", column: "" };
var Me = ["wrap", "column"];
var Te = ["onBlur", "innerHTML"];
var Ee = ["innerHTML"];
var Ce = ["innerHTML"];
var Se = ["wrap"];
var Oe = ["innerHTML"];
var $e = ["innerHTML"];
var xe = { key: 2, class: "vuecal__cell-events-count" };
var He = { key: 3, class: "vuecal__no-event" };
var We = ["onBlur", "innerHTML"];
var je = ["innerHTML"];
var Ae = { key: 2, class: "vuecal__event-time" };
var Ve = { key: 0 };
var Ye = { key: 1, class: "days-to-end" };
var Le = ["innerHTML"];
var Fe = { key: 0, class: "vuecal__scrollbar-check" };
var Be = [createBaseVNode("div", null, null, -1)];
var Ne = { weekDays: Array(7).fill(""), weekDaysShort: [], months: Array(12).fill(""), years: "", year: "", month: "", week: "", day: "", today: "", noEvent: "", allDay: "", deleteEvent: "", createEvent: "", dateFormat: "dddd MMMM D, YYYY", am: "am", pm: "pm" };
var ze = ["years", "year", "month", "week", "day"];
var Ie = new class {
  constructor(e2, t2 = false) {
    __publicField(this, "texts", {});
    __publicField(this, "dateToMinutes", (e22) => 60 * e22.getHours() + e22.getMinutes());
...
var Pe = { name: "vue-cal", components: { "vuecal-cell": ne, "vuecal-header": J, WeekdaysHeadings: I, AllDayBar: le }, provide() {
  ...
  if ("en" === this.locale) {
    const e3 = await import("./en.es-4NSJRTNB.js");
    this.texts = Object.assign({}, Ne, e3);
  } else
    (function(e3) {
      switch (e3) {
        case "./i18n/ar.json":
          return import("./ar.es-MUM5X4RD.js");
        case "./i18n/bg.json":
          return import("./bg.es-C5EBTJ3N.js");
        case "./i18n/bn.json":
          return import("./bn.es-U5GGRHF7.js");
        case "./i18n/bs.json":
          return import("./bs.es-NXAUAVR4.js");
        case "./i18n/ca.json":
          return import("./ca.es-IBGNS2OF.js");
        case "./i18n/cs.json":
          return import("./cs.es-SUAMKJAV.js");
        case "./i18n/da.json":
          return import("./da.es-SRZ7DHTC.js");
        case "./i18n/de.json":
          return import("./de.es-E2XVGGRC.js");
        case "./i18n/el.json":
          return import("./el.es-DNGAKHVQ.js");
        case "./i18n/en.json":
          return import("./en.es-4NSJRTNB.js");
        case "./i18n/es.json":
          return import("./es.es-NSE5WAGU.js");
        case "./i18n/fa.json":
          return import("./fa.es-QZXAUPHP.js");
        case "./i18n/fr.json":
          return import("./fr.es-FIQUZPKS.js");
        case "./i18n/he.json":
          return import("./he.es-MSOK5IKT.js");
        case "./i18n/hr.json":
          return import("./hr.es-HH7PMUMK.js");
        case "./i18n/hu.json":
          return import("./hu.es-RAWH6TDP.js");
        case "./i18n/id.json":
          return import("./id.es-KXA7JZGG.js");
        case "./i18n/is.json":
          return import("./is.es-RO4TGOE7.js");
        case "./i18n/it.json":
          return import("./it.es-7HASI447.js");
        case "./i18n/ja.json":
          return import("./ja.es-2O4KIETP.js");
        case "./i18n/ka.json":
          return import("./ka.es-ISHFAS4M.js");
        case "./i18n/ko.json":
          return import("./ko.es-G26BF673.js");
        case "./i18n/lt.json":
          return import("./lt.es-ZIQEPE3B.js");
        case "./i18n/mn.json":
          return import("./mn.es-LWYV6MZY.js");
        case "./i18n/nl.json":
          return import("./nl.es-WF6ZTTRH.js");
        case "./i18n/no.json":
          return import("./no.es-ELTYWMGG.js");
        case "./i18n/pl.json":
          return import("./pl.es-ZYS73D5N.js");
        case "./i18n/pt-br.json":
          return import("./pt-br.es-AYI25GT3.js");
        case "./i18n/ro.json":
          return import("./ro.es-RK4IPOLX.js");
        case "./i18n/ru.json":
          return import("./ru.es-QDX7YY3Q.js");
        case "./i18n/sk.json":
          return import("./sk.es-MXIWONQP.js");
        case "./i18n/sl.json":
          return import("./sl.es-VOXSVLFA.js");
        case "./i18n/sq.json":
          return import("./sq.es-SX263D3V.js");
        case "./i18n/sr.json":
          return import("./sr.es-BTWNNTSL.js");
        case "./i18n/sv.json":
          return import("./sv.es-G6M54RFA.js");
        case "./i18n/tr.json":
          return import("./tr.es-KBSFZZD4.js");
        case "./i18n/uk.json":
          return import("./uk.es-IFDL5NYA.js");
        case "./i18n/vi.json":
          return import("./vi.es-23KPYBFI.js");
        case "./i18n/zh-cn.json":
          return import("./zh-cn.es-WJMNNEIV.js");
        case "./i18n/zh-hk.json":
          return import("./zh-hk.es-YZYQKDSN.js");
        default:
  ...
  import("./drag-and-drop.es-VTKP5RVD.js").then((e2) => {
...
var Ue = z(Pe, [["render", function(w2, D2, g2, f2, k2, M2) {
  const T2 = resolveComponent("vuecal-header"), E2 = resolveComponent("all-day-bar"), C2 = resolveComponent("weekdays-headings"), S2 = resolveComponent("vuecal-cell");
  return openBlock(), createElementBlock("div", { class: normalizeClass(["vuecal__flex vuecal", M2.cssClasses]), column: "", ref: "vuecal", lang: g2.locale }, [createVNode(T2, { options: w2.$props, "edit-events": M2.editEvents, "view-props": { views: M2.views, weekDaysInHeader: M2.weekDaysInHeader }, "week-days": M2.weekDays, "has-splits": M2.hasSplits, "day-splits": M2.daySplits, "switch-to-narrower-view": M2.switchToNarrowerView }, createSlots({ "arrow-prev": withCtx(() => [renderSlot(w2.$slots, "arrow-prev", {}, () => [oe, de, ue])]), "arrow-next": withCtx(() => [renderSlot(w2.$slots, "arrow-next", {}, () => [he, ce, ve])]), "today-button": withCtx(() => [renderSlot(w2.$slots, "today-button", {}, () => [createBaseVNode("span", pe, toDisplayString(k2.texts.today), 1)])]), title: withCtx(() => [renderSlot(w2.$slots, "title", { title: M2.viewTitle, view: k2.view }, () => [createTextVNode(toDisplayString(M2.viewTitle), 1)])]), _: 2 }, [w2.$slots["weekday-heading"] ? { name: "weekday-heading", fn: withCtx(({ heading: e2, view: t2 }) => [renderSlot(w2.$slots, "weekday-heading", { heading: e2, view: t2 })]) } : void 0, w2.$slots["split-label"] ? { name: "split-label", fn: withCtx(({ split: e2 }) => [renderSlot(w2.$slots, "split-label", { split: e2, view: k2.view.id })]) } : void 0]), 1032, ["options", "edit-events", "view-props", "week-days", "has-splits", "day-splits", "switch-to-narrower-view"]), g2.hideBody ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", me, [createVNode(Transition, { name: `slide-fade--${k2.transitionDirection}`, appear: g2.transitions }, { default: withCtx(() => [(openBlock(), createElementBlock("div", { class: "vuecal__flex", style: { "min-width": "100%" }, key: !!g2.transitions && k2.view.id, column: "" }, [g2.showAllDayEvents && M2.hasTimeColumn && (!M2.cellOrSplitMinWidth || M2.isDayView && !g2.minSplitWidth) ? (openBlock(), createBlock(E2, normalizeProps(mergeProps({ key: 0 }, M2.allDayBar)), { event: withCtx(({ event: s2, view: i2 }) => [renderSlot(w2.$slots, "event", { view: i2, event: s2 }, () => [M2.editEvents.title && s2.titleEditable ? (openBlock(), createElementBlock("div", { key: 0, class: "vuecal__event-title vuecal__event-title--edit", contenteditable: "", onBlur: (e2) => M2.onEventTitleBlur(e2, s2), innerHTML: s2.title }, null, 40, we)) : s2.title ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__event-title", innerHTML: s2.title }, null, 8, ye)) : createCommentVNode("", true), !s2.content || M2.hasShortEvents || M2.isShortMonthView ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", { key: 2, class: "vuecal__event-content", innerHTML: s2.content }, null, 8, De))])]), _: 3 }, 16)) : createCommentVNode("", true), createBaseVNode("div", { class: normalizeClass(["vuecal__bg", { vuecal__flex: !M2.hasTimeColumn }]), column: "" }, [createBaseVNode("div", ge, [M2.hasTimeColumn ? (openBlock(), createElementBlock("div", fe, [g2.showAllDayEvents && M2.cellOrSplitMinWidth && (!M2.isDayView || g2.minSplitWidth) ? (openBlock(), createElementBlock("div", { key: 0, class: "vuecal__all-day-text", style: normalizeStyle({ height: M2.allDayBar.height }) }, [createBaseVNode("span", null, toDisplayString(k2.texts.allDay), 1)], 4)) : createCommentVNode("", true), (openBlock(true), createElementBlock(Fragment, null, renderList(M2.timeCells, (s2, i2) => (openBlock(), createElementBlock("div", { class: "vuecal__time-cell", key: i2, style: normalizeStyle(`height: ${g2.timeCellHeight}px`) }, [renderSlot(w2.$slots, "time-cell", { hours: s2.hours, minutes: s2.minutes }, () => [_e, createBaseVNode("span", be, toDisplayString(s2.label), 1)])], 4))), 128))])) : createCommentVNode("", true), g2.showWeekNumbers && M2.isMonthView ? (openBlock(), createElementBlock("div", ke, [(openBlock(), createElementBlock(Fragment, null, renderList(6, (e2) => createBaseVNode("div", { class: "vuecal__flex vuecal__week-number-cell", key: e2, grow: "" }, [renderSlot(w2.$slots, "week-number-cell", { week: M2.getWeekNumber(e2 - 1) }, () => [createTextVNode(toDisplayString(M2.getWeekNumber(e2 - 1)), 1)])])), 64))])) : createCommentVNode("", true), createBaseVNode("div", { class: normalizeClass(["vuecal__flex vuecal__cells", `${k2.view.id}-view`]), grow: "", wrap: !M2.cellOrSplitMinWidth || !M2.isWeekView, column: !!M2.cellOrSplitMinWidth }, [M2.cellOrSplitMinWidth && M2.isWeekView ? (openBlock(), createBlock(C2, { key: 0, "transition-direction": k2.transitionDirection, "week-days": M2.weekDays, "switch-to-narrower-view": M2.switchToNarrowerView, style: normalizeStyle(M2.cellOrSplitMinWidth ? `min-width: ${M2.cellOrSplitMinWidth}px` : "") }, createSlots({ _: 2 }, [w2.$slots["weekday-heading"] ? { name: "weekday-heading", fn: withCtx(({ heading: e2, view: t2 }) => [renderSlot(w2.$slots, "weekday-heading", { heading: e2, view: t2 })]) } : void 0, w2.$slots["split-label"] ? { name: "split-label", fn: withCtx(({ split: e2 }) => [renderSlot(w2.$slots, "split-label", { split: e2, view: k2.view.id })]) } : void 0]), 1032, ["transition-direction", "week-days", "switch-to-narrower-view", "style"])) : M2.hasSplits && g2.stickySplitLabels && g2.minSplitWidth ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__flex vuecal__split-days-headers", style: normalizeStyle(M2.cellOrSplitMinWidth ? `min-width: ${M2.cellOrSplitMinWidth}px` : "") }, [(openBlock(true), createElementBlock(Fragment, null, renderList(M2.daySplits, (s2, i2) => (openBlock(), createElementBlock("div", { class: normalizeClass(["day-split-header", s2.class || false]), key: i2 }, [renderSlot(w2.$slots, "split-label", { split: s2, view: k2.view.id }, () => [createTextVNode(toDisplayString(s2.label), 1)])], 2))), 128))], 4)) : createCommentVNode("", true), g2.showAllDayEvents && M2.hasTimeColumn && (M2.isWeekView && M2.cellOrSplitMinWidth || M2.isDayView && M2.hasSplits && g2.minSplitWidth) ? (openBlock(), createBlock(E2, normalizeProps(mergeProps({ key: 2 }, M2.allDayBar)), { event: withCtx(({ event: s2, view: i2 }) => [renderSlot(w2.$slots, "event", { view: i2, event: s2 }, () => [M2.editEvents.title && s2.titleEditable ? (openBlock(), createElementBlock("div", { key: 0, class: "vuecal__event-title vuecal__event-title--edit", contenteditable: "", onBlur: (e2) => M2.onEventTitleBlur(e2, s2), innerHTML: s2.title }, null, 40, Te)) : s2.title ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__event-title", innerHTML: s2.title }, null, 8, Ee)) : createCommentVNode("", true), !s2.content || M2.hasShortEvents || M2.isShortMonthView ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", { key: 2, class: "vuecal__event-content", innerHTML: s2.content }, null, 8, Ce))])]), _: 3 }, 16)) : createCommentVNode("", true), createBaseVNode("div", { class: "vuecal__flex", ref: (e2) => k2.cellsEl = e2, grow: "", wrap: !M2.cellOrSplitMinWidth || !M2.isWeekView, style: normalizeStyle(M2.cellOrSplitMinWidth ? `min-width: ${M2.cellOrSplitMinWidth}px` : "") }, [(openBlock(true), createElementBlock(Fragment, null, renderList(M2.viewCells, (s2, i2) => (openBlock(), createBlock(S2, { key: i2, options: w2.$props, "edit-events": M2.editEvents, data: s2, "cell-width": g2.hideWeekdays.length && (M2.isWeekView || M2.isMonthView) && M2.cellWidth, "min-timestamp": M2.minTimestamp, "max-timestamp": M2.maxTimestamp, "cell-splits": M2.hasSplits && M2.daySplits || [] }, { "cell-content": withCtx(({ events: i3, split: n2, selectCell: a2 }) => [renderSlot(w2.$slots, "cell-content", { cell: s2, view: k2.view, goNarrower: a2, events: i3 }, () => [n2 && !g2.stickySplitLabels ? (openBlock(), createElementBlock("div", { key: 0, class: "split-label", innerHTML: n2.label }, null, 8, Oe)) : createCommentVNode("", true), s2.content ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__cell-date", innerHTML: s2.content }, null, 8, $e)) : createCommentVNode("", true), (M2.isMonthView && !g2.eventsOnMonthView || M2.isYearsOrYearView && g2.eventsCountOnYearView) && i3.length ? (openBlock(), createElementBlock("div", xe, [renderSlot(w2.$slots, "events-count", { view: k2.view, events: i3 }, () => [createTextVNode(toDisplayString(i3.length), 1)])])) : createCommentVNode("", true), !M2.cellOrSplitHasEvents(i3, n2) && M2.isWeekOrDayView ? (openBlock(), createElementBlock("div", He, [renderSlot(w2.$slots, "no-event", {}, () => [createTextVNode(toDisplayString(k2.texts.noEvent), 1)])])) : createCommentVNode("", true)])]), event: withCtx(({ event: i3, view: n2 }) => [renderSlot(w2.$slots, "event", { view: n2, event: i3 }, () => [M2.editEvents.title && i3.titleEditable ? (openBlock(), createElementBlock("div", { key: 0, class: "vuecal__event-title vuecal__event-title--edit", contenteditable: "", onBlur: (e2) => M2.onEventTitleBlur(e2, i3), innerHTML: i3.title }, null, 40, We)) : i3.title ? (openBlock(), createElementBlock("div", { key: 1, class: "vuecal__event-title", innerHTML: i3.title }, null, 8, je)) : createCommentVNode("", true), !g2.time || i3.allDay || M2.isMonthView && (i3.allDay || "short" === g2.showAllDayEvents) || M2.isShortMonthView ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", Ae, [createTextVNode(toDisplayString(k2.utils.date.formatTime(i3.start, M2.TimeFormat)), 1), i3.endTimeMinutes ? (openBlock(), createElementBlock("span", Ve, "\xA0- " + toDisplayString(k2.utils.date.formatTime(i3.end, M2.TimeFormat, null, true)), 1)) : createCommentVNode("", true), i3.daysCount > 1 && (i3.segments[s2.formattedDate] || {}).isFirstDay ? (openBlock(), createElementBlock("small", Ye, "\xA0+" + toDisplayString(i3.daysCount - 1) + toDisplayString((k2.texts.day[0] || "").toLowerCase()), 1)) : createCommentVNode("", true)])), !i3.content || M2.isMonthView && i3.allDay && "short" === g2.showAllDayEvents || M2.isShortMonthView ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", { key: 3, class: "vuecal__event-content", innerHTML: i3.content }, null, 8, Le))])]), "no-event": withCtx(() => [renderSlot(w2.$slots, "no-event", {}, () => [createTextVNode(toDisplayString(k2.texts.noEvent), 1)])]), _: 2 }, 1032, ["options", "edit-events", "data", "cell-width", "min-timestamp", "max-timestamp", "cell-splits"]))), 128))], 12, Se)], 10, Me)])], 2)]))]), _: 3 }, 8, ["name", "appear"]), k2.ready ? createCommentVNode("", true) : (openBlock(), createElementBlock("div", Fe, Be))]))], 10, re);
}]]);

// dep:vue-cal
var vue_cal_default = Ue;
export {
